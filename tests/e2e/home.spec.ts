...
test.describe("Home page", () => {
  ...
  test("displays tool category cards in the category section", async ({
    page,
  }) => {
    // At least the "Kesehatan" category card should be visible
    await expect(
      page.getByText("Kesehatan", { exact: false }).first(),
    ).toBeVisible();
  });

  test("navigates to /kesehatan when clicking the Kesehatan category", async ({
    page,
  }) => {
    // Click the first link that contains "Kesehatan"
    const kesehatanLink = page.locator('a[href*="kesehatan"]').first();
