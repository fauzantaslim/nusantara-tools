...
export function RelatedReligiTools({ currentPath }: { currentPath: string }) {
  ...
  const tools = TOOLS.filter(t => t.category === 'Religi' && t.path !== currentPath).slice(0, 3);
  ...
  return (
    <div className="w-full border-t border-[#EDE0D0] pt-12 pb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        <div>
          <h2 className="text-2xl font-bold font-heading text-primary">Kalkulator Religi Lainnya</h2>
          <p className="text-secondary font-body mt-1 text-sm sm:text-base">Eksplorasi utilitas ibadah kami yang lain</p>
        </div>
      </div>
