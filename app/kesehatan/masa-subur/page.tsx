...
import {
  ArrowRight, Activity, Info, Calendar, CalendarDays, RefreshCcw, ShieldAlert,
  Droplet, CheckCircle2, HeartPulse, Stethoscope, ChevronLeft, ChevronRight
} from 'lucide-react';
...
import { startOfMonth, getDay, isSameMonth, isToday, format, addMonths, subMonths } from 'date-fns';
...
export default function MasaSuburCalculator() {
  ...
  const [calendarMonthStart, setCalendarMonthStart] = useState<Date>(new Date());
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    if (!firstDayOfLastPeriod) {
      setError('Harap pilih tanggal hari pertama periode menstruasi terakhir Anda.');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Terjadi kesalahan saat mengkalkulasi masa subur.');
      setResult(null);
    }
  };
  ...
  const renderCalendar = (baseDate: Date, title: string) => {
    ...
    return (
      <div className="bg-[#1A0E07]/60 p-5 rounded-3xl border border-white/10 shadow-inner mb-6">
        {title && <h4 className="text-white font-bold font-heading mb-4 text-center text-lg">{title}</h4>}
        ...
        <div className="grid grid-cols-7 gap-1 sm:gap-2">
          {dayNames.map(d => (
            <div key={d} className="text-center text-[10px] sm:text-xs font-bold text-[#7A5C42] py-2 uppercase">
              {d}
            </div>
          ))}
          ...
          {days.map((d, i) => {
            ...
            if (type === 'period') bgClass = "bg-[#9C4A2A] text-white font-bold shadow-md shadow-[#9C4A2A]/20";
            if (type === 'fertile') bgClass = "bg-[#4A7C59] text-white font-bold shadow-md shadow-[#4A7C59]/20";
            if (type === 'ovulation') bgClass = "ring-2 ring-inset ring-[#C17A3A] bg-[#C17A3A]/20 text-white font-bold";
  ...
  return (
    <div className="flex flex-col gap-12 w-full pb-20">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="mt-2">
          <h1 className="text-3xl sm:text-4xl font-black text-primary font-heading tracking-tight">Kalkulator Masa Subur</h1>
          <p className="text-base sm:text-lg text-secondary font-body mt-1">Lacak siklus bulanan & jendela ovulasi Anda</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch relative my-4">
        {/* Left Side: Input Form */}
        <Card variant="default" className="lg:col-span-5 flex flex-col gap-8 p-6 sm:p-10 border border-[#EDE0D0] shadow-xl shadow-black/[0.03] rounded-[2.5rem] bg-white relative overflow-hidden z-10 w-full lg:h-[650px]">
          ...
          <div className="relative z-10">
            <h2 className="text-2xl font-bold font-heading text-primary">Data Siklus Berjalan</h2>
            <p className="text-sm text-secondary font-body mt-2 leading-relaxed">Prediksi jadwal bulanan disesuaikan dengan pola biologis unik Anda.</p>
          </div>
          ...
          <form onSubmit={handleCalculate} className="flex flex-col gap-6 relative z-10 h-full">
            <div className="space-y-6 flex-grow">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold font-ui text-primary">Kapan hari pertama haid terakhir?</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold font-ui text-primary">Berapa lama rata-rata haid berlangsung?</label>
                <div className="relative">
                  <select
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold font-ui text-primary">Berapa panjang siklus rata-rata Anda?</label>
                <div className="relative">
                  <select
                ...
                <p className="text-xs text-secondary/70 italic mt-1 font-body">Paling umum berlangsung 28 hari.</p>
              </div>
            ...
            <div className="flex gap-3 pt-2">
              <Button type="button" variant="danger" onClick={clearForm} className="py-5 font-bold rounded-2xl whitespace-nowrap px-6 border-[#7A5C42] text-primary hover:bg-surface">
                Bersihkan
              </Button>
              <Button type="submit" variant="primary" className="py-5 text-[15px] shadow-lg hover:shadow-xl group rounded-2xl w-full">
                Kalkulasi Siklus
        ...
        <div className="lg:col-span-7 lg:h-[650px]">
          {result ? (
            <Card variant="default" className="flex flex-col relative rounded-[2.5rem] border shadow-2xl transition-all duration-700 lg:h-full animate-in fade-in zoom-in-95 bg-[#2C1A0E] text-[#F5EDE3] border-[#4A7C59]/30 ring-4 ring-inset ring-[#4A7C59]/10 overflow-hidden">
              <div className="lg:absolute lg:inset-0 lg:overflow-y-auto w-full custom-scrollbar block">
                <div className="flex flex-col p-5 sm:p-10 relative">
                  ...
                  <div className="relative z-10 flex flex-col mb-8 text-center items-center">
                    ...
                    <h3 className="text-2xl font-bold font-heading text-white">Laporan Siklus Anda</h3>
                    <p className="text-sm font-body text-[#EDE0D0]/80 mt-1 max-w-sm mb-6">
                      Berikut ini adalah rincian profil siklus menstruasi Anda untuk 6 periode ke depan.
                    </p>
                        ...
                        <div className="w-full mb-2">
                          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-left">
                            ...
                            <div className="bg-[#1A0E07]/60 p-4 rounded-2xl border border-white/5 flex flex-col justify-between hover:bg-[#1A0E07]/80 transition-colors">
                              <span className="text-[10px] sm:text-xs font-bold font-ui text-[#9C4A2A] uppercase tracking-wider mb-2">Haid Berikutnya</span>
                              ...
                              <span className="text-[10px] text-[#EDE0D0]/70">
                                {summary.nextPeriod.daysIn === 0 ? 'Hari ini' : `dalam ${summary.nextPeriod.daysIn} hari`}
                              </span>
                            </div>

                            {/* 2. Jendela Subur */}
                            <div className="bg-[#1A0E07]/60 p-4 rounded-2xl border border-white/5 flex flex-col justify-between hover:bg-[#1A0E07]/80 transition-colors">
                              <span className="text-[10px] sm:text-xs font-bold font-ui text-[#4A7C59] uppercase tracking-wider mb-2">Jendela Subur</span>
                              <span className="text-[13px] sm:text-[15px] font-bold text-white leading-tight mb-1">
                                {formatDateId(summary.nextFertileWindow.start, false).split(' ')[0]} - {formatDateId(summary.nextFertileWindow.end, false)}
                              </span>
                              <span className="text-[10px] text-[#EDE0D0]/70">
                                {summary.nextFertileWindow.daysIn === 0 ? 'Sedang berlangsung' : `dalam ${summary.nextFertileWindow.daysIn} hari`}
                              </span>
                            </div>

                            {/* 3. Ovulasi */}
                            <div className="bg-[#1A0E07]/60 p-4 rounded-2xl border border-white/5 flex flex-col justify-between hover:bg-[#1A0E07]/80 transition-colors">
                              <span className="text-[10px] sm:text-xs font-bold font-ui text-[#C17A3A] uppercase tracking-wider mb-2">Ovulasi (Puncak)</span>
                              <span className="text-sm sm:text-base font-bold text-white mb-1">
                                {formatDateId(summary.nextOvulation.date, false)}
                              </span>
                              <span className="text-[10px] text-[#EDE0D0]/70">
                                {summary.nextOvulation.daysIn === 0 ? 'Hari ini!' : `dalam ${summary.nextOvulation.daysIn} hari`}
                              </span>
                            </div>

                            {/* 4. Siklus Saat Ini */}
                            <div className="bg-[#2C1A0E] p-4 rounded-2xl border border-[#C17A3A]/30 flex flex-col justify-between shadow-inner">
                              <span className="text-[10px] sm:text-xs font-bold font-ui text-[#EDE0D0]/80 uppercase tracking-wider mb-2">Status Saat Ini</span>
                              <div className="flex items-baseline gap-1 mb-1">
                                <span className="text-xl sm:text-2xl font-black text-[#C17A3A] leading-none">Hari {summary.currentCycleDay}</span>
                              </div>
                              <span className="text-[10px] sm:text-xs font-bold text-white">{summary.currentPhase}</span>
                            </div>
                    ...
                    <div className="bg-[#1A0E07]/40 w-full rounded-2xl p-1.5 flex mt-4 border border-white/5">
                      <button
                        ...
                        className={cn(
                          ...
                          activeTab === 'table' ? "bg-[#C17A3A] text-white shadow-md" : "text-[#EDE0D0]/60 hover:text-white"
                        )}
                      >
                        Tabel Siklus
                      </button>
                      <button
                        ...
                        className={cn(
                          "flex-1 py-2.5 rounded-xl font-bold text-sm transition-all",
                          activeTab === 'calendar' ? "bg-[#C17A3A] text-white shadow-md" : "text-[#EDE0D0]/60 hover:text-white"
                        )}
                      >
                        Kalender Visual
                      </button>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full min-w-[550px] text-left text-sm text-[#F5EDE3]">
                          <thead className="bg-[#2C1A0E] font-ui uppercase text-[10px] tracking-widest text-[#EDE0D0] border-b border-white/10">
                            <tr>
                              <th className="px-5 py-4 font-bold rounded-tl-[1.5rem]">#</th>
                              <th className="px-5 py-4 font-bold">Masa Haid</th>
                              <th className="px-5 py-4 font-bold">Masa Subur</th>
                              <th className="px-5 py-4 font-bold rounded-tr-[1.5rem]">Ovulasi</th>
                            </tr>
                          </thead>
                              ...
                              <tr key={idx} className="hover:bg-white/5 transition-colors group">
                                ...
                                <td className="px-5 py-4 font-ui">
                                  <div className="flex flex-col">
                                    <span className="text-[#9C4A2A] font-bold text-xs mb-0.5">ESTIMASI</span>
                                    <span>{formatDateId(c.periodStart, false)} - {formatDateId(c.periodEnd, false)}</span>
                                  </div>
                                </td>
                                <td className="px-5 py-4 font-ui">
                                  <div className="flex flex-col">
                                    <span className="text-[#4A7C59] font-bold text-xs mb-0.5">JENDELA KESUBURAN</span>
                                    <span>{formatDateId(c.fertileWindowStart, false)} - {formatDateId(c.fertileWindowEnd, false)}</span>
                                  </div>
                                </td>
                  ...
                  {activeTab === 'calendar' && (
                    <div className="relative z-10 w-full animate-in fade-in slide-in-from-bottom-2">
                      <p className="text-sm text-[#EDE0D0]/80 mb-4 px-2 text-center">Menavigasi kalender secara visual.</p>

                      {(() => {
                        const minMonth = startOfMonth(result.cycles[0].periodStart);
                        const maxMonth = startOfMonth(result.cycles[result.cycles.length - 1].nextPeriodStart);
                        const isPrevDisabled = calendarMonthStart.getTime() <= minMonth.getTime();
                        const isNextDisabled = calendarMonthStart.getTime() >= maxMonth.getTime();
                        ...
                        return (
                          <div className="flex justify-between items-center mb-6 px-4 py-2.5 bg-[#1A0E07]/80 rounded-[1.5rem] border border-[#7A5C42]/30 shadow-sm max-w-[300px] mx-auto">
                            <button
                              onClick={() => setCalendarMonthStart(prev => subMonths(prev, 1))}
                              ...
                              className={cn(
                                ...
                                isPrevDisabled ? "text-[#EDE0D0]/30 cursor-not-allowed border border-white/5" : "text-[#EDE0D0] hover:text-white hover:bg-[#C17A3A] shadow-sm border border-[#7A5C42]/30"
                              )}
                            >
                            ...
                            <div className="text-center w-full">
                              <p className="text-base sm:text-lg font-black font-heading text-white tracking-wide uppercase">
                                {format(calendarMonthStart, 'MMMM yyyy', { locale: id })}
                              </p>
                            </div>
                            <button
                              onClick={() => setCalendarMonthStart(prev => addMonths(prev, 1))}
                              disabled={isNextDisabled}
                              className={cn(
                                "w-10 h-10 flex items-center justify-center rounded-full bg-[#2C1A0E] transition-colors focus:outline-none focus:ring-2 focus:ring-[#C17A3A] focus:ring-offset-2 focus:ring-offset-[#1A0E07]",
                                isNextDisabled ? "text-[#EDE0D0]/30 cursor-not-allowed border border-white/5" : "text-[#EDE0D0] hover:text-white hover:bg-[#C17A3A] shadow-sm border border-[#7A5C42]/30"
                              )}
                            >
                  ...
                  <div className="mt-8 mb-4 flex items-start gap-3 bg-[#9C4A2A]/10 border border-[#9C4A2A]/30 p-4 rounded-xl relative z-10 text-left">
                    ...
                    <p className="text-xs text-[#FFF0EB]/80 font-body leading-relaxed">
                      Laporan ini hanya bersifat indikasi dan hitungan matematis belaka—tidak dapat digunakan sebagai saran kontrasepsi primer atau diagnosa kondisi fertilitas absolut.
                    </p>
                  </div>
            ...
            <Card variant="default" className="flex flex-col items-center justify-center text-center p-12 h-full min-h-[400px] border-dashed border-2 bg-[#2C1A0E] border-[#7A5C42]/40 rounded-[2.5rem] transition-all relative overflow-hidden shadow-2xl text-[#F5EDE3]">
              ...
              <div className="relative z-10">
                ...
                <div className="relative z-10 w-full flex justify-center mb-2 mt-4">
                  ...
                  <Image src="/masa-subur.svg" alt="BMI Calculator Illustration" width={400} height={300} className="w-full max-w-[280px] sm:max-w-[340px] h-auto object-contain relative z-10 select-none pointer-events-none drop-shadow-2xl" priority />
                </div>
              </div>
              <h3 className="font-heading font-extrabold text-2xl sm:text-3xl text-white mb-3 tracking-tight relative z-10 px-4">Siap Memprediksi</h3>
              <p className="text-[#EDE0D0] font-body max-w-sm text-base sm:text-lg leading-relaxed relative z-10 opacity-90 px-4">
                Isi formulir hari dan panjang siklus Anda di sebelah kiri untuk melihat estimasi jendela fertilitas Anda.
              </p>
            </Card>
          )}
        </div>
            ...
            <div className="flex flex-col gap-16 lg:gap-24 relative z-10">
              ...
              <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
                ...
                <p className="text-lg text-[#EDE0D0] font-body leading-relaxed mb-10 opacity-90">
                  Keajaiban ritme hormonal dipersiapkan setiap bulannya. Memahami alur sistem siklus Anda dapat mendukung rencana fertilisasi, mendeteksi ketidakteraturan sejak dini, hingga membaca sinyal alamiah raga Anda.
                </p>
                ...
                <div className="flex p-6 sm:p-8 rounded-3xl bg-[#1A0E07]/40 border border-[#7A5C42]/30 flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 backdrop-blur-sm max-w-2xl mx-auto text-left shadow-inner">
                  ...
                  <p className="text-[#F5EDE3] font-heading font-medium text-lg leading-snug text-center sm:text-left">
                    "Pemetaan siklus masa reproduksi modern menghadirkan kendali penuh bagi perempuan dalam navigasi kesehatan holistik mereka."
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-16 mx-auto w-full">
                ...
                <section className="space-y-8">
                  <div className="flex items-center gap-4 border-b border-[#7A5C42]/30 pb-4">
                    ...
                    <h3 className="text-2xl font-bold font-heading text-white">Fase Pembentuk Siklus</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="p-6 rounded-2xl bg-[#9C4A2A]/10 border border-[#9C4A2A]/30 hover:border-[#9C4A2A]/50 transition-colors flex flex-col justify-between">
                      ...
                      <h4 className="text-xl font-bold font-heading text-white mb-2">Menstruasi</h4>
                      <p className="text-[#EDE0D0] font-body text-sm leading-relaxed opacity-80">
                        Penebalan lapisan dinding rahim terkikis & luruh. Biasanya menjadi titik nol bulan yang dirasakan 3 hingga 7 hari lamanya.
                      </p>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 transition-colors flex flex-col justify-between">
                      ...
                      <h4 className="text-xl font-bold font-heading text-white mb-2">Folikuler</h4>
                      <p className="text-[#EDE0D0] font-body text-sm leading-relaxed opacity-80">
                        Bermula seiring dengan hari haid pertama dan berakhir menjelang ovulasi. Pabrik ovarium merawat kematangan folikel sel telur secara simultan.
                      </p>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl bg-[#C17A3A]/10 border border-[#C17A3A]/30 hover:border-[#C17A3A]/50 transition-colors flex flex-col justify-between relative overflow-hidden">
                      ...
                      <h4 className="relative text-xl font-bold font-heading text-white mb-2">Ovulasi (Puncak)</h4>
                      <p className="relative text-[#EDE0D0] font-body text-sm leading-relaxed opacity-80">
                        Lepasnya telur pamungkas dari ovarium. Secara empiris muncul sekitar H-14 dari hari awal periode siklus mendatang bagi lazimnya tipe 28 hari.
                      </p>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl bg-[#4A7C59]/10 border border-[#4A7C59]/30 hover:border-[#4A7C59]/50 transition-colors flex flex-col justify-between relative overflow-hidden">
                      ...
                      <h4 className="relative text-xl font-bold font-heading text-white mb-2">Luteal</h4>
                      <p className="relative text-[#EDE0D0] font-body text-sm leading-relaxed opacity-80">
                        Dimulai saat usai ovulasi dengan durasi presisi sekitar 12-14 hari menjelang siklus anyar. Ini saat dimana fondasi rahim menebal memfasilitas implantasi.
                      </p>
                    </div>
                  </div>
                </section>
                ...
                <section className="space-y-8">
                  <div className="flex items-center gap-4 border-b border-[#7A5C42]/30 pb-4">
                    ...
                    <h3 className="text-2xl font-bold font-heading text-white">Anatomi Jendela Subur</h3>
                  </div>

                  <div className="grid md:grid-cols-2 gap-6 items-center">
                    <div>
                      <p className="text-[#EDE0D0] font-body leading-relaxed text-lg mb-4 opacity-90">
                        Jendela paling potensial berlangsung selama rentang tempo **6 hari berturut-turut** yang berakhir pas di hari puncak (ovulasi). Durasi ini bukanlah angka mati; rentang yang luas ini mengakomodir parameter biologi penyokong kehamilan.
                      </p>
                      <ul className="space-y-3 font-body text-[#EDE0D0]">
                        <li className="flex items-start gap-3 bg-white/5 p-4 rounded-xl border border-white/10">
                          ...
                          <span><strong>Masa hidup sperma:</strong> Dapat terus hidup hingga 5 hari bersembunyi di rute reproduksi sang wanita.</span>
                        </li>
                        <li className="flex items-start gap-3 bg-white/5 p-4 rounded-xl border border-white/10">
                          <CheckCircle2 className="w-5 h-5 text-[#4A7C59] shrink-0 mt-0.5" />
                          <span><strong>Masa prima telur:</strong> Begitu dirilis, sang sel telur mandiri mengambang merindukan pembuahan maksimum 24 jam.</span>
                        </li>
                      </ul>
                      <p className="text-sm font-body text-white/50 italic mt-4">Peluang optimal diperoleh melalui intiman selang beberapa waktu pada siklus jendela ini jika menargetkan pembuahan secara rasional.</p>
                    </div>
                    ...
                    <div className="bg-[#1A0E07] p-8 rounded-3xl border border-[#7A5C42]/30 shadow-sm relative overflow-hidden flex items-center justify-center min-h-[300px]">
                      ...
                      <div className="relative z-10 text-center">
                        ...
                        <h5 className="font-bold text-lg text-white font-heading tracking-wide mb-2">Golden Ratio 6-Hari</h5>
                        <p className="text-sm text-surface/80 font-body px-4">
                          Berdasarkan gabungan daya jelajah sel dan keunikan hormon masing-masing.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <section className="space-y-8">
                  <div className="flex items-center gap-4 border-b border-[#7A5C42]/30 pb-4">
                    ...
                    <h3 className="text-2xl font-bold font-heading text-white">Anomali & Batasan</h3>
                  </div>
                  ...
                  <div className="bg-[#9C4A2A]/10 border border-[#9C4A2A]/30 p-6 lg:p-8 rounded-[2rem] space-y-6 backdrop-blur-sm relative overflow-hidden">
                    ...
                    <h4 className="text-xl font-bold text-[#FFF0EB] relative z-10 flex items-center gap-2">
                      <Info className="w-5 h-5 text-[#9C4A2A]" /> Variasi Alami Itu Lumrah
                    </h4>
                    <p className="text-base font-body text-[#FFF0EB]/90 leading-relaxed mb-4 relative z-10">
                      Konstruksi kalkulator bersandar mutlak pada data observasi konvensional. Tubuh bukanlah mesin berdetik statis, keteraturan dapat mengalami manuver tajam akibat hal-hal berikut:
                    </p>

                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
                      <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <h6 className="font-bold text-white mb-1 border-l-2 border-[#C17A3A] pl-3">Psikosomatis</h6>
                        <p className="text-sm font-body text-[#EDE0D0] opacity-80 pl-3">Stres ekstrem fisik & mental menunda poros hormon sekunder.</p>
                      </div>
                      <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <h6 className="font-bold text-white mb-1 border-l-2 border-white pl-3">Berat Badan</h6>
                        <p className="text-sm font-body text-[#EDE0D0] opacity-80 pl-3">Osilasi berat drastis dalam jangka waktu teramat singkat.</p>
                      </div>
                      <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <h6 className="font-bold text-white mb-1 border-l-2 border-[#4A7C59] pl-3">Gaya Hidup</h6>
                        <p className="text-sm font-body text-[#EDE0D0] opacity-80 pl-3">Olahraga keras ala atlet pro seringkali mengerem laju rutinitas normal siklus.</p>
                      </div>
                      <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <h6 className="font-bold text-white mb-1 border-l-2 border-[#9C4A2A] pl-3">Warning Log</h6>
                        <p className="text-sm font-body text-[#EDE0D0] opacity-80 pl-3">Alat ini <strong className="text-white">tidak bisa</strong> direkayasa menjadi platform substitusi metode keluarga berencana konvensional.</p>
                      </div>
                    </div>
                  </div>
                </section>
