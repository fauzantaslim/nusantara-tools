...
describe('calculateBMI — unit tests', () => {
  ...
  it('applies correct activity multiplier — sedentary < very_active', () => {
    const sedentary = calculateBMI({ ...baseInput, activityLevel: 'sedentary' });
    const veryActive = calculateBMI({ ...baseInput, activityLevel: 'very_active' });
  ...
  it('throws when weight is too low (< 10kg equivalent)', () => {
    expect(() => calculateBMI({ ...baseInput, weight: 5 })).toThrow(/tidak valid/);
  });

  it('throws when height is too short (< 50cm)', () => {
    expect(() => calculateBMI({ ...baseInput, heightRaw1: 30 })).toThrow(/tidak valid/);
  });
});
