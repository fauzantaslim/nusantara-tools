...
export function calculateWaterIntake(input: WaterInput): WaterResult {
  ...
  if (input.activityLevel === 'moderate' || input.activityLevel === 'high' || input.exerciseDuration > 0) {
    tips.push('Aktivitas fisik ekstra meningkatkan kehilangan air melalui keringat. Pastikan Anda minum air tambahan di luar kebutuhan dasar ini setiap kali berolahraga.');
  }
  if (input.climate === 'hot' || input.climate === 'humid') {
    tips.push('Cuaca panas atau lembap mempercepat dehidrasi. Pertimbangkan untuk meningkatkan asupan air Anda hari ini.');
  }
  if (input.altitude === 'high') {
    tips.push('Di dataran tinggi, Anda mungkin kehilangan air lebih cepat karena pernapasan. Tingkatkan hidrasi Anda.');
  }
  if (input.caffeine || input.alcohol) {
    tips.push('Kafein atau alkohol dapat bertindak sebagai diuretik ringan yang menyebabkan Anda lebih sering buang air kecil. Minumlah ekstra untuk menyeimbangkannya.');
  }
  if (input.highProtein) {
    tips.push('Diet tinggi protein memerlukan lebih banyak air untuk metabolisme. Pastikan Anda mencukupi kebutuhan cairan Anda.');
  }
  if (input.pregnant) {
    tips.push('Kebutuhan hidrasi meningkat secara signifikan selama kehamilan dan menyusui. Silakan konsultasi dengan tenaga medis.');
  }
  if (input.sick) {
    tips.push('Demam atau penyakit akan membakar lebih banyak cairan. Terus tingkatkan asupan air Anda untuk pemulihan optimal.');
  }

  // Fallback default tip if none applied
  if (tips.length === 0) {
    tips.push('Anda memiliki kondisi standar. Asupan cairan dasar ini sangat cocok untuk memulai hidrasi harian Anda yang sehat.');
  }
  ...
  const schedule: HydrationSchedule[] = distribution.map((slot) => {
    ...
    return {
      ...
      // Round appropriately
      amount: input.unit === 'liter' || input.unit === 'cups' 
  ...
  return {
    baseIntakeMl,
    totalIntakeMl,
    convertedIntake: input.unit === 'liter' || input.unit === 'cups' ? Math.round(convertedIntake * 10) / 10 : Math.round(convertedIntake),
    unitLabel,
    schedule,
