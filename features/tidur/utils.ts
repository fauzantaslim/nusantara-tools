...
function getQuality(cycles: number): { quality: SleepQuality; color: 'red' | 'orange' | 'blue' | 'green' } {
  if (cycles <= 2) return { quality: 'Buruk', color: 'red' };
  if (cycles === 3 || cycles === 4) return { quality: 'Kurang Optimal', color: 'orange' };
...
export function calculateSleepCycles(input: SleepInput): SleepResult {
  const { mode, targetTime, latencyMinutes, cycleLengthMinutes, maxCycles, timeFormat } = input;
