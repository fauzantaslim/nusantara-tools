...
export function calculateBMI(input: BMIInput): BMIResult {
  const { system, weight, heightRaw1, heightRaw2 = 0, gender, age, activityLevel } = input;
...
if (roundedScore < 18.5) {
  category = 'Kurus';
  insight = 'Berat badan kamu masih di bawah ideal. Coba tambah asupan makan bergizi dan jangan sering skip makan ya.';
} else if (roundedScore <= 24.9) {
  category = 'Normal';
  insight = 'Nice! Berat badan kamu udah di range ideal. Tinggal dijaga aja pola makan dan aktivitasnya.';
} else if (roundedScore <= 29.9) {
  category = 'Berlebih';
  insight = 'Berat badan kamu mulai naik nih. Bisa mulai dari hal kecil kayak kurangi gula dan lebih aktif bergerak.';
} else {
  category = 'Obesitas';
  insight = 'Berat badan kamu sudah cukup tinggi. Sebaiknya mulai atur pola makan dan kalau perlu konsultasi ke tenaga kesehatan.';
}
  ...
  let bodyFatPercentage = (1.20 * roundedScore) + (0.23 * age) - (10.8 * genderValue) - 5.4;
