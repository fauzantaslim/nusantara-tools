...
export function getDayType(date: Date, cycles: CycleResult[]): DayType {
  for (const cycle of cycles) {
    ...
    // Check period
    if (
      (isSameDay(date, cycle.periodStart) ||
    ...
    // Check fertile
    if (
      (isSameDay(date, cycle.fertileWindowStart) ||
        isAfter(date, cycle.fertileWindowStart)) &&
...
export function getCurrentCycleInfo(
  ...
  for (let i = 0; i < cycles.length; i++) {
    if (
      (isSameDay(todayStart, cycles[i].periodStart) ||
        isAfter(todayStart, cycles[i].periodStart)) &&
  ...
  if (
    isBefore(nextOvulation, todayStart) &&
    activeCycleIdx + 1 < cycles.length
  ) {
  ...
  if (
    isBefore(nextFertileWindowEnd, todayStart) &&
    !isSameDay(nextFertileWindowEnd, todayStart) &&
    activeCycleIdx + 1 < cycles.length
  ) {
  ...
  if (
    (isSameDay(todayStart, nextFertileWindowStart) ||
      isAfter(todayStart, nextFertileWindowStart)) &&
    (isSameDay(todayStart, nextFertileWindowEnd) ||
      isBefore(todayStart, nextFertileWindowEnd))
  ) {
