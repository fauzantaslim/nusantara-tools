...
export function CategorySection() {
  ...
  return (
    <section
      id="kategori"
      className="py-24 flex flex-col gap-10 relative z-20 border-t border-muted/50 max-w-7xl mx-auto w-full"
    >
      {/* Header & Controls */}
      <div className="flex flex-col items-center text-center w-full max-w-3xl mx-auto">
        ...
        <p className="text-secondary font-body mt-4 sm:mt-5 text-lg sm:text-xl leading-relaxed w-full px-4">
          Kumpulan perangkat digital pintar yang dirancang khusus untuk
          mempermudah aktivitas finansial, kesehatan, religi, hingga rutinitas
          harian Anda.
        </p>
      </div>
                ...
                {displayedTools.map((tool) => {
                  ...
                  return (
                    <Link
                      href={tool.path}
                      key={tool.id}
                      className="group flex items-start gap-4 p-3 -mx-3 rounded-[1rem] hover:bg-surface border border-transparent hover:border-[#EDE0D0]/60 transition-all outline-none focus-visible:ring-2 focus-visible:ring-accent-1/50"
                    >
                      <div
                        className={cn(
                          "w-12 h-12 rounded-xl flex items-center justify-center shrink-0 shadow-sm border border-white/60",
                          tool.bg,
                        )}
                      >
